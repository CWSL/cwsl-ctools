...
##############################################################################
#
# Description: Aggregate data along the meridional (latitudinal) axis
#              
...
function usage {
    echo "Aggregate data long the meridional (latitudinal) axis."
    echo " "
    echo "USAGE: bash $0 method infile outfile"
    echo "   method:     Aggregation method."
    echo "               Choices: mermin, mermax, mersum, mermean, meravg,"
    echo "                        mervar, merstd, merpctl,N (where N is the percentile)"
    echo "   infile:     Input file name"
    echo "   outfile:    Output file name"
    echo " "
    echo "   e.g. bash $0 mermean indata.nc outdata.nc"
    echo "   e.g. bash $0 merpctl,90 indata.nc outdata.nc"
    exit 1
}
