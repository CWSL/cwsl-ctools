...
##############################################################################
#
# Description: Aggregate data along the time axis
#              
...
function usage {
    echo "Aggregate data along time axis."
    echo " "
    echo "USAGE: bash $0 method infile outfile"
    echo "   method:     Aggregation method."
    echo "               Choices: timmin, timmax, timsum, timmean, timavg,"
    echo "                        timvar, timvar1, timstd, timstd1," 
    echo "                        timpctl,N (where N is the percentile)"
    echo "   infile:     Input file name"
    echo "   outfile:    Output file name"
    echo " "
    echo "   e.g. bash $0 timmean indata.nc outdata.nc"
    echo "   e.g. bash $0 timpctl,90 indata.nc outdata.nc"
    exit 1
}
