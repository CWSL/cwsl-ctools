...
##############################################################################
#
# Description: Aggregate data along the zonal (longitudinal) axis
#              
...
function usage {
    echo "Aggregate data long the zonal (longitudinal) axis."
    echo " "
    echo "USAGE: bash $0 method infile outfile"
    echo "   method:     Aggregation method."
    echo "               Choices: zonmin, zonmax, zonsum, zonmean, zonavg,"
    echo "                        zonvar, zonstd, zonpctl,N (where N is the percentile)"
    echo "   infile:     Input file name"
    echo "   outfile:    Output file name"
    echo " "
    echo "   e.g. bash $0 zonmean indata.nc outdata.nc"
    echo "   e.g. bash $0 zonpctl,90 indata.nc outdata.nc"
    exit 1
}
