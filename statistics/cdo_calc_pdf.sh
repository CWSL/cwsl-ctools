...
function usage {
    echo "Calculates a pdf (probability density function)"
    echo " "
    echo "USAGE: bash $0 bin_list infile outfile"
    ...
    echo " "
    echo "   e.g. bash $0 bin_list indata.nc outdata.nc"
    exit 1
}
...
basedir="${infile%$infilename}"
echo
tmpfile="${basedir}${basefilename}_histcount.nc"
echo $tmpfile
cdo -O histcount,$options $infile $tmpfile
cdo div $tmpfile -vertsum $tmpfile $outfile
rm ${tmpfile}
